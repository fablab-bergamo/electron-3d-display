...
def emit_header(factors):
    lines = [
        ...
        "",
        "inline constexpr int kAtomSizeCalibCount = %d;" % len(factors),
        "",
        "inline constexpr orb_real_t kAtomSizeCalibFactor[%d] = {" % len(factors),
    ]
...
def main():
    ...
    h_path = os.path.join(root, 'src', 'physics', 'atom_size_calib.h')
